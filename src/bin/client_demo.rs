...
async fn main() {
    ...
    file.write_block(0, test_data).await.expect("Failed to write block");
    ...
    let multi_file = client.open("/multi_block.txt").await.expect("Failed to open file");
    ...
    for (i, data) in block_data.iter().enumerate() {
        multi_file.write_block(i as u64, data).await
            .expect(&format!("Failed to write block {}", i));
        println!("Wrote block {}: {} bytes", i, data.len());
    }
    
    for (i, expected) in block_data.iter().enumerate() {
        let actual = multi_file.read_block(i as u64).await
            .expect(&format!("Failed to read block {}", i));
    ...
    let success_rate = if stats.total_requests > 0 {
        ((stats.total_requests - stats.failed_requests) as f64 / stats.total_requests as f64) * 100.0
    } else {
        100.0
    };
    ...
    println!("Concurrent test: {}/{} successful\n", successful, num_concurrent);

    // Final statistics
    println!("=== Final Statistics ===");
    let final_stats = client.get_connection_stats().await;
    println!("Total operations: {}", final_stats.total_requests);
    println!("Total data transferred: {} bytes", 
             final_stats.bytes_sent + final_stats.bytes_received);
    
    // Cleanup
    println!("\n=== Cleanup ===");
    client.delete(file_path).await.expect("Failed to delete test file");
    client.delete("/multi_block.txt").await.expect("Failed to delete multi-block file");
    for i in 0..num_concurrent {
        let path = format!("/concurrent_file_{}.txt", i);
